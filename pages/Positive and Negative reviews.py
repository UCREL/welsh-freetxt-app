...
def read_file(fname, file_source):
    ...
    if file_name.endswith('.txt'):
        data = open(fname, 'r', encoding='cp1252', errors='ignore').read().split('\n') if file_source=='example' else fname.read().decode('utf8').split('\n')
        ...
        data = pd.read_csv(fname, sep='\t', encoding='cp1252', errors='ignore') if file_source=='example' else pd.read_csv(fname, sep='\t', encoding='cp1252')
    else:
